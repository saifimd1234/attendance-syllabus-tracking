...
export default function Navbar() {
    ...
    const { user, checkAuth, logout, isAuthChecking } = useStore();

    useEffect(() => {
        checkAuth();
    }, []);

    // Don't show navbar if on landing page or if not logged in
    const isLandingPage = pathname === '/';
    if (isLandingPage || (!user && !isAuthChecking)) return null;
    ...
    return (
        <>
            {/* Global style for padding when sidebar is visible */}
            <style jsx global>{`
                @media (min-width: 640px) {
                    main { padding-left: 5rem !important; }
                }
            `}</style>
